  ...
  <div class="bg-gray-900 py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-6 text-center lg:px-8">
      ...
      <ul role="list" class="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
        <li v-for="service in services" :key="service.name" class="rounded-2xl bg-gray-800 px-8 py-10 cursor-pointer" @click="openServiceModal(service.id)">
          <img class="mx-auto h-48 w-48 rounded-full md:h-56 md:w-56" :src="service.imageUrl" alt="" />
          <h3 class="mt-6 text-base font-semibold leading-7 tracking-tight text-white">{{ service.title }}</h3>
